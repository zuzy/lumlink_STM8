...
void read_dht11(void)
{
    ...
    if(dht_start())
    {
		...
		tmp_string[0] = string[4];
		tmp_string[1] = string[5];
		tmp_string[2] = '\0';
		hum_string[0] = string[0];
		hum_string[1] = string[1];
		hum_string[2] ='\0';
		UART1_SendString("\t温度：",8);
		UART1_SendString(tmp_string,3);
		UART1_SendString(" 摄氏度",8);
		
		UART1_SendString("\t湿度：",8);
		UART1_SendString(tmp_string,3);
		UART1_SendString("%\r\n",4);
    }  
    LED_G = 1;
